...
class AIJFile:
    ...
    def __init__(self, filename, filter_type, target_star_dict, ref_star_dict, point_format=None, **kwargs):
...
class Target:
    ...
    def getMagData(self, filter_type, return_1D_lists=None, **kwargs): #returns HJD, mag, dmag, and plot color lists. Maybe I should make this a thing in the AIJFile class also so I can just call that from this function for each file?
        ...
        Kwargs:
        :param use_files: list containing which files to use for calculations, plots, etc., if not all files should be included.
        :type use_files: list containing AIJFile and/or OtherDataFile objects
        :param use_ref_stars: a list of reference star names (see ``ref_star_dict``) to use for calculation of magnitude, if use of only some reference stars is desired
        :type use_ref_stars: list 
        """
    ...
    def getFourierFit(self, filter_type, lineFmt=None, showPlot=None, **kwargs):
        """
        ...
        popt, pcov = fourierFit(phase, Mag, dMag, plot_title=self.name, plot_labels=labs, **kwargs)
        if showPlot:
    ...
    def calcPeriod(self, filter_type, **kwargs):
        ...
        """
        HJD, Mag, dMag, pltColor, isCharError = self.getMagData(filter_type, return_1D_lists=True, **kwargs)
        calcP = getPeriod(HJD, Mag, dMag, **kwargs)
